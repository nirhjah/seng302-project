...
public class PersonalAggregatedStatisticsControllerTest {
    ...
    @Test
    public void testUserStatsPage() throws Exception {
        mockMvc.perform(get("/myStats"))
                .andExpect(status().isOk())
                .andExpect(view().name("userTeamSummaryStats"));
    }

    @WithMockUser(username = "johndoe@example.com", password = "Password123!", roles = "USER")
    @Test
    public void testUserTeamActivityStats() throws Exception {
        Team team = new Team("Team 900", "Programming", new Location(null, null, null, "aud", null, "pls"));
        teamRepository.save(team);
        mockMvc.perform(get("/myActivityStats").param("teamID", team.getTeamId().toString()))
                .andExpect(status().isOk())
                .andExpect(view().name("userActivityTeamStats"));
    }
}
