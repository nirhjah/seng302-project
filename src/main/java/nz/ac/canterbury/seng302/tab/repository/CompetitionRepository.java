...
public interface CompetitionRepository extends CrudRepository<Competition, Long> {
    ...
    @Query("""
            ...
            FROM Competition c
              ...
              AND ((:now >= c.competitionStart) AND (:now <= c.competitionEnd)))
              """)
    ...
    @Query("""
            ...
            FROM Competition c
              WHERE ((:#{#filteredSports.size}=0 OR (c.sport in :filteredSports)))
              """)
    Page<Competition> findAllCompetitionsBySports(Pageable pageable, @Param("filteredSports") List<String> filteredSports);
